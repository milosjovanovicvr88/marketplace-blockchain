...
const AxiesFilter = (props:any) => {
  ...
  const onChangeMenuItem = (e: { value: any; }) => {
    let temps=[...items];
    if (e.value==="Lowest Price"){ 
      temps.sort((a: { price: number; }, b: { price: number; }) => {        
  ...
  return (
    <>      
      <div className="w-full min-w-[100vw] bg-[#242735] m-0">
        <div className='relative w-full h-full'>
          <div className='left-part border-r border-gray-700 text-14 text-white font-normal'>
            <div className="left-content">
              ...
              <ExpandDiv filtertitle="Class" items={props.items} filterItems={setItems}/>
              <ExpandDiv filtertitle="Counts" items={props.items} filterItems={setItems}/>
              <ExpandDiv filtertitle="Other" items={props.items} filterItems={setItems}/>
            </div>
              ...
              <div className="flex jsutify-center">
                {
                  showListType ?
                    <div className="flex flex-wrap justify-center">
                      {items.map((item: { id: any; }, _index : number) => <TileItem key={_index} data={item} />)}
                    </div>
                  :                      
                    <div className="w-full">
                      {items.map((item: { id: any; }, _index : number) => <ListItem key={_index} data={item} />)}
                    </div>
                }
              </div>
            </div>
